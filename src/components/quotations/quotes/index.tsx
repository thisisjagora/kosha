...
const Quotes:FC<HTMLAttributes<HTMLDivElement>> = ({...props}) => <div {...props} className={cn("flex flex-col gap-2 justify-between p-4 bg-white-100 shadow-sm rounded-xl flex-1 w-full max-w-[400px] sm:max-w-[270px] sm:min-w-[270px] lg:max-w-[270px] max-h-[350px] hover:cursor-pointer", props.className)} />
...
const QuotesAmount = ({ amount }: {amount: number}) => <div className="p-1 min-w-[80px] max-w-max text-center bg-primary rounded-3xl"><P className="font-dm-sans text-sm text-white-100">{formatCurrency(amount)}</P></div>
