            ...
            >
                  ...
                  <DialogContent className="lg:hidden left-0 translate-x-0 max-w-[280px] h-full bg-primary border-0 border-transparent shadow-transparent p-0 flex flex-col justify-start rounded-0">
                        ...
                        <Column className="py-6 gap-6">
                              {
                                    SIDE_NAV_MENU_DATA.map((item, indx) => <NavItem key={item.title + indx} data={{...item, onClick: () => setIsOpen(false), isActive: isActiveRoute(item.route) || (item.route === Routes.root && sequenceRoutes.includes(path)) || (item.route === Routes.messages && !!messagesRouteMatch)}} />)
                              }
                        </Column>
                  </DialogContent>
            </Dialog>
