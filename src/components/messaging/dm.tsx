...
const DM: FC<HTMLAttributes<HTMLDivElement>> = ({ ...props }) => (
  <Column
    {...props}
    className={cn(
      "w-full h-full p-4 relative overflow-hidden",
      props.className
    )}
  />
);

const DMHeader: FC<{ type?: string; name: string; time: string }> = ({
  type = "Book a move",
  name,
  time,
}) => {
    ...
    <Row className="w-full py-[11px]">
      <Row className="items-center flex-1">
        ...
        <Column className="flex-1 gap-0">
          <P className="font-dm-sans text-primary font-bold text-xl leading-[24px]">
            {name}
          </P>
          <P className="font-dm-sans text-primary-foreground text-xs">{time}</P>
        </Column>
      </Row>
    </Row>
  );
...
const DMFooter: FC<HTMLAttributes<HTMLDivElement>> = ({ ...props }) => (
  <Row {...props} className={cn("items-center gap-1", props.className)} />
);

export { DM, DMHeader, DMFooter };
